...
static td_tap_t eng_btn2_f5_state=INIT_TAP_STATE;void eng_btn2_f5_finished(tap_dance_state_t*s,void*d){eng_btn2_f5_state.state=cur_dance(s);switch(eng_btn2_f5_state.state){case TD_SINGLE_TAP:tap_code16(LSA(KC_7));break;case TD_SINGLE_HOLD: tap_code16(KC_BTN2); break;case TD_DOUBLE_TAP:tap_code(KC_F5);break;default:break;}}void eng_btn2_f5_reset(tap_dance_state_t*s,void*d){eng_btn2_f5_state.state=TD_NONE;}
...
const uint16_t PROGMEM combo_rcs_t_z[]={TD(TD_HM_WWW),LT(2,KC_END),COMBO_END};
const uint16_t PROGMEM combo_ltb_gl_z[]={LT(3,KC_PGDN),KC_LEFT,COMBO_END};
const uint16_t PROGMEM combo_rtb_gr_z[]={KC_DOWN,KC_RGHT,COMBO_END};
const uint16_t PROGMEM combo_ntb_gu_z[]={LT(3,KC_PGDN),KC_DOWN,COMBO_END};
const uint16_t PROGMEM combo_ctb_gd_z[]={KC_LEFT,KC_RGHT,COMBO_END};
const uint16_t PROGMEM combo_lctl_e_z[]= {KC_DEL,LT(2,KC_TAB),COMBO_END};
const uint16_t PROGMEM combo_lalt_f4_z[]={KC_MUTE,KC_DEL,COMBO_END};
const uint16_t PROGMEM combo_volup[]={TD(TD_HM_WWW),KC_MUTE,COMBO_END};
const uint16_t PROGMEM combo_voldn[]={LT(2,KC_END),KC_DEL,COMBO_END};
const uint16_t PROGMEM combo_rsg_left[]={LT(2,KC_END),MT(MOD_RALT,KC_PGUP),COMBO_END};
const uint16_t PROGMEM combo_rsg_rght[]={KC_DEL,KC_UP,COMBO_END};
const uint16_t PROGMEM combo_lgui_tab[]={MT(MOD_RALT,KC_PGUP),KC_UP,COMBO_END};
...
const uint16_t PROGMEM combo_space_1[]={MT(MOD_LALT,KC_ESC),LT(1,KC_SPC),COMBO_END};
const uint16_t PROGMEM combo_space_2[]={LT(2,KC_ESC),LT(1,KC_SPC),COMBO_END};
combo_t key_combos[]={
    ...
    COMBO(combo_lctl_e_z,LCTL(KC_E)),
    COMBO(combo_lalt_f4_z,LALT(KC_F4)),
    ...
    COMBO(combo_space_1,KC_SPC),COMBO(combo_space_2,KC_SPC),
};
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS]={
	[0] = LAYOUT_65_xt_ansi_blocker_tsangan(
		TD(TD_HM_WWW),KC_MUTE,     TD(TD_EGUI_GH),LT(1,KC_1),LT(2,KC_2),MT(MOD_LCTL,KC_3),MT(MOD_LSFT,KC_4),MT(MOD_LALT,KC_5),MT(MOD_LGUI,KC_6),KC_7,KC_8,KC_9,LT(1,KC_0),MT(MOD_LCTL,KC_MINS),MT(MOD_LSFT,KC_EQL),KC_BSPC,TD(TD_HM_WWW),
		LT(2,KC_END), KC_DEL,      LT(2,KC_TAB),LT(1,KC_Q),LT(3,KC_W),KC_E,LT(3,KC_R),KC_T,LT(2,KC_Y),KC_U,LT(3,KC_I),KC_O,LT(2,KC_P),KC_LBRC,KC_RBRC,LT(2,KC_BSLS),TD(TD_PU_HM_L2),
		MT(MOD_RALT,KC_PGUP),KC_UP,LT(3,KC_GRV),KC_A,LT(2,KC_S),KC_D,KC_F,LT(1,KC_G),KC_H,KC_J,LT(1,KC_K),KC_L,LT(3,KC_SCLN),TD(TD_QUOT_AL_SQUO),LT(3,KC_ENT),TD(TD_PD_ED_RCTL),
		LT(3,KC_PGDN),KC_DOWN,     KC_LSFT,LT(3,KC_Z),LT(1,KC_X),KC_C,LT(2,KC_V),LT(3,KC_B),KC_N,KC_M,KC_COMM,LT(2,KC_DOT),TD(TD_SLSH_CL_QUEST),KC_RSFT,KC_UP,MT(MOD_RALT,KC_END),
		KC_LEFT,   KC_RGHT,        MT(MOD_LCTL,KC_PDOT),LT(2,KC_BSPC),MT(MOD_LALT,KC_ESC),LT(1,KC_SPC),LT(2,KC_ESC),KC_LEFT,KC_DOWN,KC_RGHT
    ),
	[1] = LAYOUT_65_xt_ansi_blocker_tsangan(
		KC_TRNS,   KC_TRNS,        LCTL(KC_K),KC_F1,KC_F2,KC_F3,KC_F4,KC_F5,KC_F6,KC_F7,KC_F8,KC_F9,KC_F10,KC_F11,KC_F12,KC_END,QK_COMBO_TOGGLE,
		KC_TRNS,   KC_TRNS,        LCTL(LGUI(KC_LEFT)),LCTL(LGUI(KC_RGHT)),LGUI(KC_UP),RCS(KC_PGUP),RCS(KC_PGDN),LCTL(KC_T),KC_TRNS,KC_TRNS,LCTL(LGUI(KC_D)),KC_MYCM,RCS(KC_UP),LCTL(KC_LEFT),LCTL(KC_RGHT),RCS(KC_DOWN),QK_COMBO_ON,
		KC_TRNS,   KC_TRNS,        LCTL(KC_F),LGUI(KC_LEFT),LGUI(KC_DOWN),LGUI(KC_RGHT),RCS(KC_TAB),LCTL(KC_W),LCTL(KC_TAB),LCTL(LGUI(KC_LEFT)),LCTL(LGUI(KC_F4)),LCTL(LGUI(KC_RGHT)),TD(TD_SELTEX_BFALL),TD(TD_SELTEX_AFALL),LGUI(KC_TAB),QK_COMBO_OFF,
		KC_TRNS,   KC_TRNS,        LSA(KC_P),RSG(KC_LEFT),RSG(KC_RGHT),MEH(KC_TAB),LCA(KC_TAB),LGUI(KC_TAB),LCTL(KC_N),KC_APP,LALT(KC_ESC),LSA(KC_ESC),LALT(KC_LEFT),LALT(KC_RGHT),LCTL(KC_EQL),KC_TRNS,
		KC_TRNS,   KC_TRNS,        LALT(KC_ENT),KC_F5,LGUI(KC_H),KC_SPC,KC_PSCR,LCTL(KC_0),LCTL(KC_MINS),LCTL(KC_1)
    ),    
	[2] = LAYOUT_65_xt_ansi_blocker_tsangan(
		LCTL(KC_1),LCTL(KC_0),     LALT(KC_F4),KC_MYCM,KC_CALC,LCTL(KC_F),KC_PSLS,LALT(KC_LEFT),LALT(KC_RGHT),LSA(KC_8),KC_LSFT,LCTL(KC_C),LCTL(KC_V),LSFT(KC_TAB),KC_TAB,LCTL(KC_A),KC_MYCM,
		KC_TRNS,   LCTL(KC_EQL),   KC_DEL,KC_P7,KC_P8,KC_P9,KC_PAST,RCS(KC_T),KC_TRNS,LSA(KC_7),KC_INS,KC_TRNS,KC_TRNS,RCS(KC_HOME),RCS(KC_END),LCTL(KC_E),RCS(KC_PGUP),
		KC_TRNS,   LCTL(KC_MINS),  KC_NUM,KC_P4,KC_P5,KC_P6,KC_PMNS,KC_TRNS,LCTL(KC_H),LSA(KC_9),LALT(KC_GRV),LGUI(KC_L),LALT(KC_UP),LALT(KC_DOWN),LCTL(KC_L),RCS(KC_PGDN),
		KC_TRNS,   KC_TRNS,        KC_P0,KC_P1,KC_P2,KC_P3,KC_PPLS,LCTL(KC_B),LSA(KC_1),LSA(KC_2),LSA(KC_Q),LGUI(KC_DOT),LCTL(KC_B),KC_WWW_HOME,LCTL(KC_T),RCS(KC_T),
		KC_TRNS,   KC_TRNS,        KC_WWW_HOME,RCS(KC_T),LCTL(KC_A),KC_PENT,LCTL(KC_H),RCS(KC_TAB),LCTL(KC_W),LCTL(KC_TAB)
    ), 
	[3] = LAYOUT_65_xt_ansi_blocker_tsangan(
		KC_TRNS,   KC_TRNS,        KC_F11,LALT(KC_LEFT),LALT(KC_RGHT),KC_F13,KC_F14,KC_F15,KC_F16,KC_F17,KC_F18,KC_F19,KC_F20,KC_F21,KC_F22,KC_F23,KC_F24,
		KC_TRNS,   KC_TRNS,        KC_HOME,KC_PGUP,KC_UP,LALT(KC_ESC),LSA(KC_ESC),LCTL(KC_EQL),LCTL(KC_1),KC_MPRV,KC_F,KC_MPLY,KC_C,KC_LSFT,LSA(KC_8),LSA(KC_7),MEH(KC_TAB),
		KC_TRNS,   KC_TRNS,        KC_F7,KC_PGDN,KC_DOWN,MEH(KC_TAB),LCA(KC_TAB),LCTL(KC_MINS),LCTL(KC_0),KC_MNXT,KC_MRWD,KC_MFFD,LALT(KC_GRV),LSA(KC_9),LALT(KC_F4),LCA(KC_TAB),
		KC_TRNS,   KC_TRNS,        KC_END,KC_LEFT,KC_RGHT,KC_TRNS,KC_TRNS,QK_BOOT,KC_VOLD,KC_VOLU,LALT(KC_LEFT),LALT(KC_RGHT),LCTL(KC_Z),RCS(KC_Z),KC_TRNS,KC_TRNS,
		KC_TRNS,   KC_TRNS,        KC_TRNS,KC_SLSH,LGUI(KC_X),KC_MUTE,KC_MPLY,KC_TRNS,KC_TRNS,KC_TRNS
    ),
};